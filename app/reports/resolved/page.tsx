...
export default function ResolvedReportsPage() {
  ...
  const loadReports = useCallback(async () => {
    try {
      setLoading(true)
      const data = await apiClient.getReports({ status: "resolved" })
      setReports(data)
    } catch (error) {
      console.error("Error loading reports:", error)
      toast({
        title: "Error Loading Reports",
        description: error instanceof Error ? error.message : "Unable to load resolved reports. Please try again.",
        variant: "destructive",
        duration: 5000,
      })
    } finally {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="p-8 text-center">
          <div className="space-y-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Loading Resolved Reports</h2>
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 md:pl-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">Reports that have been successfully addressed</p>
            </div>
            <Button
              onClick={loadReports}
              variant="outline"
        ...
        {reports.length === 0 ? (
          <Card className="p-12 text-center">
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-800">No Resolved Reports</h3>
                <p className="text-gray-600">
                  There are currently no resolved reports. Check back later or view all reports.
                </p>
              </div>
            </div>
          </Card>
